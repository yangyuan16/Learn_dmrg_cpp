...
if __name__ == "__main__":
    ...
    print(filename)
    # load the data
    df_yx = pd.read_csv(filename, header=None, sep='\t',encoding='utf-8')
    df_yx.rename(columns={0: "site1", 1: "site2", 2: "site3", 3: "site4", 4: "corre"}, inplace=True)
    print(df_yx.head())
    print(df_yx.tail())
    print(len(df_yx))
    df_yx = df_yx[(df_yx["site3"]-df_yx["site1"]) % (Lz * Ly) == 0] 
    df_yx.sort_values(["site3"],inplace=True)
    site1 = df_yx["site1"].values
    site3 = df_yx["site3"].values
    corre_yx = np.abs(np.array(df_yx["corre"].values))
    r_yx = np.array((site3 - site1) / (Lz * Ly))
    print(r_yx)
    #============= YZ pairing data ===========================
    ...
    print(filename)
    # load the data
    df_yz = pd.read_csv(filename, header=None, sep='\t',encoding='utf-8')
    df_yz.rename(columns={0: "site1", 1: "site2", 2: "site3", 3: "site4", 4: "corre"}, inplace=True)
    print(df_yz.head())
    print(df_yz.tail())
    print(len(df_yz))
    df_yz = df_yz[(df_yz["site3"]-df_yz["site1"]) % (Lz * Ly) == 0] 
    df_yz.sort_values(["site3"],inplace=True)
    site1 = df_yz["site1"].values
    site3 = df_yz["site3"].values
    corre_yz = np.abs(np.array(df_yz["corre"].values))
    r_yz = np.array((site3 - site1) / (Lz * Ly))
    print(r_yz)
    #============= YZ pairing data ===========================
    ...
    print(filename)
    # load the data
    df_zx = pd.read_csv(filename, header=None, sep='\t',encoding='utf-8')
    df_zx.rename(columns={0: "site1", 1: "site2", 2: "site3", 3: "site4", 4: "corre"}, inplace=True)
    print(df_zx.head())
    print(df_zx.tail())
    print(len(df_zx))
    df_zx = df_zx[(df_zx["site3"]-df_zx["site1"]) % (Lz * Ly) == 0] 
    df_zx.sort_values(["site3"],inplace=True)
    site1 = df_zx["site1"].values
    site3 = df_zx["site3"].values
    corre_zx = np.abs(np.array(df_zx["corre"].values))
    r_zx = np.array((site3 - site1) / (Lz * Ly))
    print(r_zx)
    #============= Green Function data ========================
    ...
    Lyx, = ax1.plot(r_yx,corre_yx,label="$<\Delta_i^{yx} \Delta_j^{yx}>$",ls="-",lw=1.5,color="magenta",
             marker='o',alpha=1,markersize=10,markeredgewidth=1.5, markeredgecolor="magenta",
             markerfacecolor='None')
    
    Lyz, = ax1.plot(r_yz,corre_yz,label="$<\Delta_i^{yz} \Delta_j^{yz}>$",ls="-",lw=1.5,color="cyan",
             marker='o',alpha=1,markersize=10,markeredgewidth=1.5, markeredgecolor="cyan",
             markerfacecolor='None')
    
    Lzx, = ax1.plot(r_zx,corre_zx,label="$<\Delta_i^{zx} \Delta_j^{zx}>$",ls="-",lw=1.5,color="olive",
             marker='o',alpha=1,markersize=10,markeredgewidth=1.5, markeredgecolor="olive",
             markerfacecolor='None')
    ...
    legend1=plt.legend(handles=[Lzz,Lyy,Lxx,Lyx,Lyz,Lzx,Lgreenfun], loc = 4, bbox_to_anchor=(0.28, 0.08),
