...
if __name__ == "__main__":
    ...
    r_Jz11, corre_Jz11 = get_data(Lz=Lz,Ly=Ly,Lx=Lx,dop=dop,t=t,J=J,Jz=1.1,dim=dim)
    r_Jz12, corre_Jz12 = get_data(Lz=Lz,Ly=Ly,Lx=Lx,dop=dop,t=t,J=J,Jz=1.2,dim=dim)
    ...
    L06, = ax1.plot(r_Jz06,corre_Jz06,label="Jz={}".format(0.6),ls="-",lw=1.5,color="r",
             marker='s',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='w')
    L07, = ax1.plot(r_Jz07,corre_Jz07,label="Jz={}".format(0.7),ls="-",lw=1.5,color="r",
             marker='s',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='w')
    L08, = ax1.plot(r_Jz08,corre_Jz08,label="Jz={}".format(0.8),ls="-",lw=1.5,color="r",
             marker='s',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='w')
    L09, = ax1.plot(r_Jz09,corre_Jz09,label="Jz={}".format(0.9),ls="-",lw=1.5,color="r",
             marker='s',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='w')    
    ...
    L20, = ax1.plot(r_Jz20,corre_Jz20,label="Jz={}".format(2.0),ls="-",lw=1.5,color="blue",
             marker='^',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='k')
    L40, = ax1.plot(r_Jz40,corre_Jz40,label="Jz={}".format(4.0),ls="-",lw=1.5,color="blue",
             marker='v',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",
             markerfacecolor='k')
    
    ####图例设置
    legfont = {'family' : 'Times New Roman','weight' : 'normal','size': 16, }###图例字体的大小###ncol 设置列的数量，使显示扁平化，当要表示的线段特别多的时候会有用
    legend1=plt.legend(handles=[L01,L02,L03,L04,L05,L06,L07,L08,L09,L10,L20,L40], loc = 4, bbox_to_anchor=(0.38, 0.18),
