...
if __name__ =="__main__":
    ...
    Lyy, = ax4.plot(Jz_list,K_sc_yy,label=label,ls="-",lw=2,color="red",
             marker='s',alpha=1,markersize=7,markeredgewidth=1.5, markeredgecolor="red", markerfacecolor='w')
    error_range_dn = np.array([0.15,0.15,0.15,0.15,0.15,0.15,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25]) # 下置信度
    error_range_up = np.array([0.15,0.15,0.15,0.15,0.15,0.15,0.25,0.25,0.5,0.5,0.5,0.5,0.5,0.5,0.5]) # 上置信度
    error_range = [error_range_dn,error_range_up]
    plt.errorbar(Jz_list,K_sc_yy,yerr=error_range,fmt=" ", ecolor="red",elinewidth=2.3,ms=7,mfc="w",mec="red",
                            capsize=3,capthick=1.5, )
    #----------------------------------------------------
    # fmt: 数据点的标记样式以及相互之间连接线样式, marker
    # ecolor: 误差棒的线条颜色
    # elinewidth: 误差棒的线条粗细
    # capsize: 误差棒边界横杠的大小
    # capthick: 误差棒边界横杠的厚度
    ...
    Lzz, = ax4.plot(Jz_list,K_sc_zz,label=label,ls="-",lw=2,color="blue",
             marker='s',alpha=1,markersize=7,markeredgewidth=1.5, markeredgecolor="blue", markerfacecolor='w')
    error_range_dn = np.array([0.25,0.25,0.25,0.25,0.25,0.25,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15]) # 下置信度
    error_range_up = np.array([0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15]) # 上置信度
    error_range = [error_range_dn,error_range_up]
    plt.errorbar(Jz_list,K_sc_zz,yerr=error_range,fmt=" ", ecolor="blue",elinewidth=2.3,ms=7,mfc="w",mec="red",
                            capsize=3,capthick=1.5, )
    #----
    label = "$K_{CDW}$"
    ...
    error_range_dn = np.array([0.25,0.25,0.25,0.25,0.25,0.25,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15]) # 下置信度
    error_range_up = np.array([0.20,0.20,0.20,0.20,0.20,0.25,0.25,0.15,0.15,0.15,0.15,0.15,0.15,0.15,0.15]) # 上置信度
    error_range = [error_range_dn,error_range_up]
    plt.errorbar(Jz_list,K_cdw,yerr=error_range,fmt=" ", ecolor="green",elinewidth=2.3,ms=7,mfc="w",mec="red",
                            capsize=3,capthick=1.5, )
    #    
    ####图例设置
    legfont = {'family' : 'Times New Roman','weight' : 'normal','size': 16, }###图例字体的大小###ncol 设置列的数量，使显示扁平化，当要表示的线段特别多的时候会有用
    ...
    #
    plt.savefig("E:\\WORK\\Work\\Project\\La3Ni2O7\\datashow_datpy\\figs\\fig4_dop72_Yy_Zz_D.eps",
