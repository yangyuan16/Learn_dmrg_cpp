# 比较不同 Jz 下 power law fitting 出来的指数：
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    #========= plot curve
    L1, = ax1.plot(Jz_list,Kcdw_list,label=r"$K_{CDW}$",ls="-",lw=1.5,color="magenta",
                   marker='v',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",markerfacecolor='w')
    L2, = ax1.plot(Jz_list,Ksc_yy_list,label=r"$K_{sc}^{yy}$",ls="-",lw=1.5,color="blue",
                   marker='o',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",markerfacecolor='w')
    L3, = ax1.plot(Jz_list,Ksc_zz_list,label=r"$K_{sc}^{zz}$",ls="-",lw=1.5,color="green",
                   marker='s',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="k",markerfacecolor='w')
    L4, = ax1.plot([0.1,8],[2,2],label=r"$K_{sc}^{zz}$",ls="--",lw=1.5,color="k",
    ...
    legend1=plt.legend(handles=[L1,L2,L3], loc = 4, bbox_to_anchor=(0.68, 0.42),
