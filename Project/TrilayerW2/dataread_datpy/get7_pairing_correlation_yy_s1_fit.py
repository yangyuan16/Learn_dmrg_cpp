...
def plot_expfit(df):
    ...
    plt.title("In-Up:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("In-Up:Sign of YY Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_yy_s1.dat"
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_yy_s1_fit.parquet" 
