...
def plot_corre_sign(df):
    ...
    plt.title("Dn-layer:Sign of XX Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_xx_la2.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[0,3,8,13,16,19,22,23]] # exponential fit
    df_cut_pow = df.loc[[0,3,8,13,16,19]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_xx_la2_fit.parquet" 
