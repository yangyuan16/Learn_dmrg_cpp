...
def plot_expfit(df):
    ...
    xi = -1/slope 
    label_data = "$<\Delta_i^{yy} \Delta_j^{xx}>$"
    ...
    plt.title("YY-XX:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("Sign of YY-XX Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
