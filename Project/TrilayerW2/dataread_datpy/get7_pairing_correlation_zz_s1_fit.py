...
def plot_expfit(df):
    ...
    xi = -1/slope 
    label_data = "$<\Delta_i^{zz} \Delta_j^{zz}>$"
    ...
    plt.title("Shift1:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("Shift1:Sign of ZZ Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_zz_s1.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[4,10,13,22]] # exponential fit
    df_cut_pow = df.loc[[1,4,10,13]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_zz_s1_fit.parquet"
