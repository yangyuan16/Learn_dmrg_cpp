...
def density_along_x_Lz3(df,Ly,Lz): # 沿着 x 方向的 electron density
    ...
    df_out["n1mn3"] = (df_out["dy0"] + df_out["dy1"] - df_out["dy4"] - df_out["dy5"])/2
    df_out["n1pn3"] = (df_out["dy0"] + df_out["dy1"] + df_out["dy4"] + df_out["dy5"])/4
...
def plot_loopJz_density_curve(Jz):
    if Lz == 2:
        df_x = density_along_x_Lz2(df=df, Ly=Ly, Lz=Lz)
    elif Lz == 3:
        df_x = density_along_x_Lz3(df=df, Ly=Ly, Lz=Lz)
    else:
        raise "wrong of Lz"
    #----plot density curve----------------
    if Lz == 2:
        ...
        #plot_density_curve(r=df_x["rmean"],density=df_x["dymean"],label_y='<ni>(mean)',Jz=Jz)
        #
        #plot_density_curve(r=df_x["rmean"],density=df_x["n1mn3"],label_y='<n1>-<n3>',Jz=Jz)
        #plot_density_curve(r=df_x["rmean"],density=df_x["n1pn3"],label_y='<n1>+<n3>',Jz=Jz)
        #
        n1_total = df_x["dy0"].sum() + df_x["dy1"].sum()
        n3_total = df_x["dy4"].sum() + df_x["dy5"].sum()
        print("Jperp=%.2f ,n1_total=%.2f ,n3_total=%.2f ,n1-n3=%.2f"%(Jz,n1_total,n3_total,n1_total-n3_total))


    else:
        raise Exception("wrong input of Lz")
    return
#
if __name__ == "__main__":
    ...
    t = 3.0
    J = 1.0
    Jz_list = [12.0,15.0,18.0,21.0,24.0,48.0]
    dim = 6000 # dim cutoff
    for it in range(len(Jz_list)):
        ...
        filename = "/home/yangyuan16/WORK/Work/Project/t3.00_J1.00_Jz%.2f_dim6000/measurement_electron_density.dat"%Jz
        # load the data
