...
def plot_expfit(df):
    ...
    plt.title("In-Dn:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("In-Dn:Sign of YY Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_yy_s2.dat"
    ...
    print(df.head())
    df = df[(df["site3"]+2-df["site1"]) % (Lz * Ly) == 0] 
    ...
    r = np.array((site3 + 2 - site1) / (Lz * Ly))
    print(r)
    #
    df["r"] = r
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[5,9,14,18,22,26,30]] # exponential fit
    df_cut_pow = df.loc[[0,5,9,14,18]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_yy_s2_fit.parquet" 
