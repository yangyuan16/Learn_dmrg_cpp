...
def plot_corre_sign(df):
    ...
    plt.title("Dn-layer:Sign of YY Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_yy_la2.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[3,6,9,12,15,18,21,24]] # exponential fit
    df_cut_pow = df.loc[[0,3,6,9,12,15,18]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_yy_la2_fit.parquet" 
