...
if __name__ == "__main__":
    ...
    Dim= 10000
    #load data
    file_up = "\\measurement_green_function_s1.dat" # up-layer
    file_in = "\\measurement_green_function.dat" # inner-layer
    file_dn = "\\measurement_green_function_s2.dat" # down-layer
    #
    ...
    L_up, = ax1.plot(df_up["r"],df_up["corre_abs"],label="$<c_i\cdot c_j>$ (Up-layer)",ls="-",lw=1.5,color="red",
             marker='o',alpha=1,markersize=12,markeredgewidth=1.5, markeredgecolor="red",
             markerfacecolor='None')
    #
    L_in, = ax1.plot(df_in["r"],df_in["corre_abs"],label="$<c_i\cdot c_j>$ (In-layer)",ls="-",lw=1.5,color="blue",
             marker='o',alpha=1,markersize=12,markeredgewidth=1.5, markeredgecolor="blue",
             markerfacecolor='None')
    #
    L_dn, = ax1.plot(df_dn["r"],df_dn["corre_abs"],label="$<c_i\cdot c_j>$ (Dn-layer)",ls="-",lw=1.5,color="green",
