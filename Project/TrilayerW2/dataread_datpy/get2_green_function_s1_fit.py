...
def plot_expfit(df):
    ...
    plt.title("Up-layer:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
if __name__ == "__main__":
    ...
    print(len(df))
    #================选定某些数据点=============================
    df_cut_exp = df.loc[[4,8,14,21,27]] # for exponential fit 
    df_cut_pow = df.loc[[1,4,7,11,19,26]] # for power law fit
