...
#--------------------------------------------
def df_along_Lx_Lz2Ly2(df,Lz,Ly):
    if Ly == 2:
        print("---------df_res0---------")
...
#
def df_along_Lx_Lz3Ly2(df,Lz,Ly):
    print("---------df_res0---------")
    df_res0 = df[df["site"]%(Lz*Ly)==0]
    df_res0["r"] = range(1,len(df_res0)+1) 
    print(df_res0.head())
...
def build_df_res2(Lx,df_res2):
    ...
    return df_res2
#
def build_df_res3(Lx,df_res3):
    x = df_res3["r"].values
    logr = entropy_logr(Lx=Lx,x=x)
    df_res3["logr"] = logr
    df_res3.index = range(len(df_res3))
    return df_res3
#
...
def plot_entropyfit_res3(df):
    ...
    intercept = df["intercept"].values[0]
    label_data = "entropy"
    L1, = ax1.plot(logr,entropy,label=label_data,ls="-",lw=1.5,color="magenta",
...
if __name__ == "__main__":
    ...
    filepath2 = "\\t%.2f_J%.2f_Jz%.2f_dim%d_ent\\entanglement\\entropy" % (t, J, Jz, dim)
    ...
    print(df.tail())
    print(len(df))
    df_res0, df_res1, df_res2, df_res3,df_res4,df_res5 = df_along_Lx_Lz3Ly2(df=df,Lz=Lz,Ly=Ly)
    #--------------------------------------------------------------------
    '''
    # choose df_res1
    ...
    #-----------------------------------------------------------
    # choose df_res3
    df_res3 = build_df_res3(df_res3=df_res3,Lx=Lx)
    print('---------------------')
    print(df_res3.head())
    print(df_res3.tail())
    # cut half of Lx of df_res2
    df_res3_cut = df_res3.iloc[2:np.int32(Lx/2),:]
    df_res3_cut.index = range(len(df_res3_cut))
    print("df_res3_cut:\n", df_res3_cut)
    #             选定某些数据点
    df_cut = df_res3_cut.loc[[7,12,18]] 
    df_fit = get_fit_data(df0=df_res3_cut, df_cut=df_cut)
    print("df_fit:\n",df_fit)
    #
    plot_entropyfit_res3(df=df_fit)
    #   
    Issave = True 
    if Issave:
        filepath4 = "\\t%.2f_J%.2f_Jz%.2f_dim%d_ent\\entanglement" % (t, J, Jz, dim)
        filepath5 = "\\measurement_entropy_fit.parquet" 
        savepath = workpath + filepath1 + filepath4 + filepath5
        df_fit.to_parquet(savepath)