...
def plot_expfit(df):
    ...
    plt.title("Shift2:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("Shift2:Sign of ZZ Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_zz_s2.dat"
    ...
    df = df[(df["site3"]-1-df["site1"]) % (Lz * Ly) == 0] 
    ...
    r = np.array((site3 -1 - site1) / (Lz * Ly))
    print(r)
    #
    df["r"] = r
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[0,13,21,29]] # exponential fit
    df_cut_pow = df.loc[[0,5,13]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_zz_s2_fit.parquet"
