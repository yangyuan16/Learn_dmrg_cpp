...
def plot_expfit(df):
    ...
    plt.title("Dn-layer:Lx=%d,dop=%.4f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
if __name__ == "__main__":
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[3,10,18,25]] # for exponential fit 
    df_cut_pow = df.loc[[0,4,10,18,25]] # for power law fit
    #df_cut = df
