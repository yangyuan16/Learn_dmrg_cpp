...
if __name__ == "__main__":
    ...
    Dim= 10000
    #load data
    file_up = "\\measurement_pairing_yy_la1.dat" # up-layer
    file_in = "\\measurement_pairing_yy.dat" # inner-layer
    file_dn = "\\measurement_pairing_yy_la2.dat" # down-layer
    #
    ...
    L_up, = ax1.plot(df_up["r"],df_up["corre_abs"],label="$<\Delta^{yy}\Delta^{yy}>$ (Up-layer)",ls="-",lw=1.5,color="red",
             marker='o',alpha=1,markersize=12,markeredgewidth=1.5, markeredgecolor="red",
             markerfacecolor='None')
    #
    L_in, = ax1.plot(df_in["r"],df_in["corre_abs"],label="$<\Delta^{yy}\Delta^{yy}>$ (In-layer)",ls="-",lw=1.5,color="blue",
             marker='o',alpha=1,markersize=12,markeredgewidth=1.5, markeredgecolor="blue",
             markerfacecolor='None')
    #
    L_dn, = ax1.plot(df_dn["r"],df_dn["corre_abs"],label="$<\Delta^{yy}\Delta^{yy}>$ (Dn-layer)",ls="-",lw=1.5,color="green",
