...
def plot_corre_sign(df):
    ...
    plt.title("Sign of ZZ Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[4,8,12,18,22,26]] # exponential fit
    df_cut_pow = df.loc[[0,4,8,12,18]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_zz_fit.parquet"
