...
def plot_expfit(df):
    ...
    plt.title("Up-Dn:Lx=%d,dop=%.4f,t=%.2f,J=%.2f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("Up-Dn:Sign of ZZ Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_zz_la13.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[3,6,14,17,20,23,26]] # exponential fit
    df_cut_pow = df.loc[[0,3,6,14,17,20]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_zz_la13_fit.parquet"
