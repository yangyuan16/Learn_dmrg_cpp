...
def plot_expfit(df):
    ...
    plt.title("Dn-layer:Lx=%d,dop=%.4f,t=%.2f,J=%.2f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("Dn-layer:Sign of Green Fun. Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_green_function_s2.dat"
    ...
    print(len(df))
    #================选定某些数据点=============================
    df_cut_exp = df.loc[[2,10,13,17,20,24,27]] # for exponential fit 
    df_cut_pow = df.loc[[0,2,10,13,17,20]] # for power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_green_function_s2_fit.parquet"
