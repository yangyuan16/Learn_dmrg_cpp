...
def plot_corre_sign(df):
    ...
    plt.title("Up-layer Sign of Spin Corre. Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_spin_correlation_s1.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[3,8,10,15,20,25]] # exponential fit
    df_cut_pow = df.loc[[0,3,5,10,15,20]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_spin_correlation_s1_fit.parquet"
