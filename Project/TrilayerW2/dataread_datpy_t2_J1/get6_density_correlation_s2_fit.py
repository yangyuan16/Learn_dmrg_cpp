...
def plot_corre_sign(df):
    ...
    plt.title("Dn-layer:Sign of Density Corre. Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_density_correlation_s2.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[4,8,15,19,23,27]] # for exponential fit 
    df_cut_pow = df.loc[[0,4,8,15,19]] # for power law fit
    #df_cut = df
    ...
    Issave = True 
    if Issave:
        filepath4 = "\\measurement_density_correlation_s2_fit.parquet" 
