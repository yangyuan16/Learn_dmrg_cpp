...
def plot_expfit(df):
    ...
    plt.title("In-Dn:Lx=%d,dop=%.4f,t=%.2f,J=%.2f,Jz=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,Jz,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
...
def plot_corre_sign(df):
    ...
    plt.title("In-Dn:Sign of ZZ Pairing, Jz=%.2f"%Jz, fontsize=25)
    plt.show()
    return
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_zz_la23.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[3,7,11,15,21,25]] # exponential fit
    df_cut_pow = df.loc[[0,3,7,11,15]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_zz_la23_fit.parquet"
