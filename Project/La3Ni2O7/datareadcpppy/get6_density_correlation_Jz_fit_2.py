# 对比不同 Jz 值下的density correlation function 的power 指数
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    for it in range(len(Jz_list)):
        Jz = Jz_list[it]
        df = get_data(Lz=Lz,Ly=Ly,Lx=Lx,dop=dop,t=t,J=J,Jz=Jz,dim=dim)
    ...
    #========= plot curve
    L1, = ax1.plot(Jz_list,K_list,label=r"$K_{CDW}$",ls="-",lw=1.5,color="magenta",
