...
//			Infinite_test( para, lsys, lns );
		...
		num = 0;
		number = 0;
	        initialstate = 6000;  //change this !!! number of kept states for the first sweep from left to right
	        site_start = 1;  //para.untruncated_site/2-1;//left end site of the sweep region
        	site_end = para.Total_N - 2;  //para.untruncated_site/2;//right end site of the sweep region
		...
		Finite_Sweep( para, site_start, site_end, sign, initialstate, step, precision ); //change 'site_start' to the continue site number !!!
	        ...
	        for( step = 13; step < max_num; step++ ) {  //change the initial "step" number !!!!!!
...
inline void Sweep::Measurement_LeftToRight( Parameter &para, const double &precision ) {
	...
	SpinCorrelation_xo *spin_xo = new SpinCorrelation_xo( lsys, para, *sup_measure );
        delete spin_xo;
	...
	delete green;
        ...
        GreenFunction_xo *green_xo = new GreenFunction_xo( lsys, para, *sup_measure );
        delete green_xo;
...
//    delete sqdensity;
        ...
        DensityCorrelation_xo *density_xo = new DensityCorrelation_xo( lsys, para, *sup_measure );
        delete density_xo;
        
	BondSpin *bondspin = new BondSpin( lsys, para, *sup_measure);
        delete bondspin;

        BondDensity *bonddensity = new BondDensity( lsys, para, *sup_measure);
        delete bonddensity;

        BondHopping *bondhopping = new BondHopping( lsys, para, *sup_measure);
        delete bondhopping;
...
inline void Sweep::Measurement_LeftToRight_shifted( Parameter &para, const double &precision ) {
	...
	delete pairing;
        ...
        PairingCorrelation_zz_xo *pairing_zz_xo = new PairingCorrelation_zz_xo( lsys, para, *sup_measure );
        delete pairing_zz_xo;

	PairingCorrelation_yy_xz *pairing_yy_xz = new PairingCorrelation_yy_xz( lsys, para, *sup_measure);
        delete pairing_yy_xz;
	
	PairingCorrelation_xx_zo *pairing_xx_zo = new PairingCorrelation_xx_zo( lsys, para, *sup_measure);
        delete pairing_xx_zo;

        PairingCorrelation_xx_xo *pairing_xx_xo = new PairingCorrelation_xx_xo( lsys, para, *sup_measure);
        delete pairing_xx_xo;
