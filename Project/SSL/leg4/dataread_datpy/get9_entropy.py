...
def plot_entropy(df0,df1,df2,df3):
    ...
    r = df0["r"].values
    L1, = ax1.plot(r,df0["entropy"],label="tnnn=%.2f,Jnnn=%.2f"%(t1,J1),ls="-",lw=1.5,color="red",
    ...
    r = df1["r"].values
    L2, = ax2.plot(r,df1["entropy"],label="tnnn=%.2f,Jnnn=%.2f"%(t1,J1),ls="-",lw=1.5,color="blue",
    ...
    r = df2["r"].values
    L3, = ax3.plot(r,df2["entropy"],label="tnnn=%.2f,Jnnn=%.2f"%(t1,J1),ls="-",lw=1.5,color="green",
    ...
    r = df3["r"].values
    L4, = ax4.plot(r,df3["entropy"],label="tnnn=%.2f,Jnnn=%.2f"%(t1,J1),ls="-",lw=1.5,color="magenta",
...
def plot_entropy_v2(df0,df1,df2,df3):
    ...
    #
    r = df3["r"].values
    L4, = ax1.plot(r,df3["entropy"],label="case3",ls="-",lw=1.5,color="magenta",
             marker='o',alpha=1,markersize=6,markeredgewidth=1.5, markeredgecolor="magenta",
             markerfacecolor='None')
    #
    legfont = {'family' : 'Times New Roman','weight' : 'normal','size': 22, }###图例字体的大小###ncol 设置列的数量，使显示扁平化，当要表示的线段特别多的时候会有用
...
if __name__ == "__main__":
    ...
    filepath2 = "\\t%.2f_J%.2f_t%.2f_J%.2f_dim%d_ent\\entanglement\\entropy" % (t, J, t1, J1, dim)
    ...
    #=================================================
    c_list =[4,] 
    g_list = [1.5,1.6,1.7,1.8,1.9,2.0]
    color = "red"
    for c in c_list:
        for g in g_list:
