...
if __name__ == "__main__":
    ...
    # choose df_res1
    df_res1 = build_df_res(df_res=df_res1,Lx=Lx)
    print('---------------------')
    print(df_res1.head())
    print(df_res1.tail())
    ...
    print("df_res1_cut:\n", df_res1_cut)
    #================选定某些数据点=============================
    df_cut = df_res1_cut.loc[[2,9,17]] 
    df_fit = get_fit_data(df0=df_res1_cut, df_cut=df_cut)
    print("df_fit:\n",df_fit)
    #
    plot_entropyfit(df=df_fit)
    #   
    Issave = True 
    if Issave:
        filepath4 = "\\t%.2f_J%.2f_t%.2f_J%.2f_dim%d_ent\\entanglement" % (t, J, t1, J1, dim)
