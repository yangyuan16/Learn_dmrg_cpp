# 对 green function 进行指数型和power law 型的拟合
import numpy as np
import pandas as pd
...
def plot_expfit(df):
    ...
    L1, = ax1.plot(r,corre_abs,label=label_data,ls="-",lw=1.5,color="red",
             marker='o',alpha=1,markersize=10,markeredgewidth=1.5, markeredgecolor="k",
...
def plot_corre_sign(df):                                                                                                    
    ...
    plt.title("Sign of Green Fun. t=%.2f,J=%.2f,t2=%.2f,J2=%.2f"%(t,J,t1,J1), fontsize=20)                                                                 
    plt.show()                                                                                                              
    return  
#
if __name__ == "__main__":
    ...
    print(len(df))
    #================选定某些数据点=============================
    df_cut_exp = df.loc[[3,6,9,12,15,18,21,24,27]] # for exponential fit 
    df_cut_pow = df.loc[[3,6,9,12,15,18]] # for power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_green_function_fit.parquet"
