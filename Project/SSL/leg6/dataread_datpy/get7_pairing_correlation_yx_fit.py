...
def plot_expfit(df):
    ...
    xi = -1/slope 
    label_data = "$<\Delta_i^{yx} \Delta_j^{yx}>$"
    ...
    plt.title("Lx=%d,dop=%.4f,t=%.2f,J=%.2f,t2=%.2f,J2=%.2fDim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,t1,J1,dim), fontsize=25)
    plt.show()
    return
#
def plot_powfit(df):
    ...
    #ax1.set_yticks([-0.1,0,0.5,1])
    #ax1.text(0.3,-0.05, r'$\mathrm{(a)}$', fontsize=18)
    plt.title("Lx=%d,dop=%.4f,t=%.2f,J=%.2f,t2=%.2f,J2=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,t1,J1,dim), fontsize=20)
    plt.show()
    return
#
def plot_corre_sign(df):                                                                                                    
    corre = list(df["corre"].values)                                                                                        
    #--------------------------------------------------                                                                     
    sign_list = []                                                                                                          
    ...
    ax3.plot(df['r'].values,sign_list,label=r"G",ls="--",lw=1.5,color="blue",                                               
    ...
    plt.title("Sign of YX Pairing, t=%.2f,J=%.2f,t2=%.2f,J2=%.2f"%(t,J,t1,J1), fontsize=20)                                                                
    plt.show()                                                                                                              
    return                                                                                                                  
#  
#
if __name__ == "__main__":
    ...
    J1 = 12.0
    dim = 6000 # dim cutoff
    workpath = "E:\\WORK\\Work\\Project\\SSLattice\\leg6"
    filepath1 = "\\data_dmrgcpp\\Lz%d_Ly%d_Lx%d\\dop%g" % (Lz, Ly, Lx,dop)
    filepath2 = "\\t%.2f_J%.2f_t%.2f_J%.2f_dim%d" % (t, J, t1, J1, dim)
    filepath3 = "\\measurement_pairing_yy_xx.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[8,11,14,17,20,23]] # exponential fit
    df_cut_pow = df.loc[[3,8,11,14,17]] # power law fit
    ...
    plot_expfit(df=df)
    plot_powfit(df=df)
    plot_corre_sign(df=df)
    #---------------save data --------------------------
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_yy_xx_fit.parquet"
