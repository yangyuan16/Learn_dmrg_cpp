...
def plot_expfit(df):
    ...
    xi = -1/slope 
    label_data = "$<\Delta_i^{nnn} \Delta_j^{nnn}>$"
...
def plot_powfit(df):
    ...
    #ax1.set_yticks([-0.1,0,0.5,1])
    #ax1.text(0.3,-0.05, r'$\mathrm{(a)}$', fontsize=18)
    plt.title("Lx=%d,dop=%.4f,t=%.2f,J=%.2f,t2=%.2f,J2=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,t1,J1,dim), fontsize=25)
    plt.show()
    return
#
def plot_corre_sign(df):                                                                                                    
    ...
    plt.title("Sign of NNN Pairing, t=%.2f,J=%.2f,t2=%.2f,J2=%.2f"%(t,J,t1,J1), fontsize=25)                                                                
    plt.show()                                                                                                              
    return                                                                                                                  
#  
#
if __name__ == "__main__":
    ...
    filepath3 = "\\measurement_pairing_yy_dia1.dat"
    ...
    print(len(df))
    #================选定某些数据点==========================
    df_cut_exp = df.loc[[2,8,14,20,26]] # exponential fit
    df_cut_pow = df.loc[[0,2,8,10,12,14,16]] # power law fit
    ...
    Issave = True
    if Issave:
        filepath4 = "\\measurement_pairing_yy_dia1_fit.parquet"
