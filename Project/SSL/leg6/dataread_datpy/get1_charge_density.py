...
def plot_density_curve(r,density, label_y):
    ...
    plt.title("Lx=%d,dop=%.4f,t=%.2f,J=%.2f,t2=%.2f,J2=%.2f,Dim=%d"%(Lx,dop/(Lz*Ly*Lx),t,J,t1,J1,dim), fontsize=23)
    plt.show()
    return
#
def density_along_x_Ly4(df,Ly,Lz): # 沿着 x 方向的 electron density
    ...
    return df_out
#
def density_along_x_Ly6(df,Ly,Lz): # 沿着 x 方向的 electron density
    df_out = pd.DataFrame(columns = ["r0","dy0","r1","dy1","r2","dy2","rmean","dymean"])
    #
    df_y0 = df[df['site'] % (Ly * Lz) ==0]
    df_out["r0"] = df_y0["site"].values
    ...
    density_mean = (np.array(df_out["dy0"].values) + np.array(df_out["dy1"].values) + 
                    ...
                    np.array(df_out["dy4"].values) + np.array(df_out["dy5"].values)) / (Lz*Ly)
    #
    df_out["rmean"] = r_mean
    df_out["dymean"] = density_mean
    #
...
if __name__ == "__main__":
    ...
    if Ly == 2:
        ...
        df_x = density_along_x_Ly6(df=df, Ly=Ly, Lz=Lz)
    else:
        raise "wrong of Ly"
    #----plot density curve----------------
    if Ly == 2:
