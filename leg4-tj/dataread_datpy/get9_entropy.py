...
def plot_entropy(df0,df1,df2,df3):
    ...
    label_y = "entropy"
    #plt.yscale("log")
    ax4.set_xlabel(label_x, size= 20)
    ax4.set_ylabel(label_y, size= 20)
    ax4.tick_params(labelsize = 20) # 设置坐标刻度对应数字的大小
    #ax4.set_xlim([0,8])
    #ax4.set_ylim([-0.1,1])
    #ax4.set_xticks([0,2,4,6,8])
    #ax4.set_yticks([-0.1,0,0.5,1])
    #ax4.text(0.3,-0.05, r'$\mathrm{(a)}$', fontsize=18)
    plt.title("case4",fontsize=20)    
    #--------------------------------------------------------    
    plt.show()
    return
#
